...
function PreviewOverview() {
  const tiles = [
    { label: 'Queue Depth', val: '14', color: 'var(--primary)' },
    { label: 'Processing',  val: '6',  color: 'var(--tertiary)' },
    { label: 'Completed',   val: '4.8K', color: 'var(--secondary)' },
    { label: 'Failed',      val: '278', color: 'var(--error)' },
  ];
  const events = ['LAPTOP-7F2A  •  completed', 'CHAIR-09BC  •  processing', 'CPU-A112  •  queued', 'MONITOR-5C3D  •  failed'];
  return (
    <div style={{ display: 'flex', gap: 12 }}>
      <div style={{ flex: 1 }}>
        ...
        <div style={{ background: 'var(--surf1)', border: '1px solid var(--border)', borderRadius: 4, padding: '10px 12px' }}>
          <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--outline)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Live Metrics — Throughput</div>
          <div style={{ height: 48, display: 'flex', alignItems: 'flex-end', gap: 3, paddingBottom: 2 }}>
            {[22, 28, 18, 35, 42, 30, 25, 38, 44, 36, 50, 42, 38, 46, 40, 34, 48, 44, 52, 46].map((h, i) => (
              <div key={i} style={{ flex: 1, height: `${h}%`, borderRadius: '2px 2px 0 0', background: i % 5 === 4 ? 'var(--primary)' : 'rgba(192,193,255,0.25)', transition: 'height 0.3s' }} />
            ))}
          </div>
        </div>
      </div>
      <div style={{ width: 160, flexShrink: 0, background: 'var(--surf1)', border: '1px solid var(--border)', borderRadius: 4, padding: '10px 12px' }}>
        ...
        {events.map((e, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 7 }}>
            <div style={{ width: 5, height: 5, borderRadius: '50%', flexShrink: 0, background: e.includes('completed') ? 'var(--secondary)' : e.includes('failed') ? 'var(--error)' : e.includes('processing') ? 'var(--tertiary)' : 'var(--outline)' }} />
            <span style={{ fontSize: 10, color: 'var(--on-variant)', fontFamily: "'JetBrains Mono', monospace", whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{e}</span>
          </div>
        ))}
      </div>
    </div>
...
function PreviewCatalog() {
  const products = ['CPU Quantum X9', 'Laptop Pro Assembly', 'Mechanical Keyboard', 'NVMe SSD 2TB'];
  return (
    <div style={{ display: 'flex', gap: 10, height: 160 }}>
      <div style={{ width: 140, background: 'var(--surf1)', border: '1px solid var(--border)', borderRadius: 4, overflow: 'hidden' }}>
        ...
        {products.map((p, i) => (
          <div key={p} style={{
            ...
            borderLeft: i === 1 ? '3px solid var(--primary)' : '3px solid transparent',
            background: i === 1 ? 'rgba(192,193,255,0.06)' : 'transparent',
            fontSize: 11, color: i === 1 ? 'var(--primary)' : 'var(--on-variant)',
      ...
      <div style={{ flex: 1, background: 'var(--surf1)', border: '1px solid var(--border)', borderRadius: 4, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--on-surface)' }}>Laptop Pro Assembly</div>
        <div style={{ display: 'flex', gap: 6 }}>
          <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 6px', borderRadius: 3, background: 'rgba(78,222,163,0.1)', color: 'var(--secondary)', textTransform: 'uppercase' }}>IN STOCK</span>
          <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 6px', borderRadius: 3, background: 'rgba(192,193,255,0.1)', color: 'var(--primary)', textTransform: 'uppercase', fontFamily: "'JetBrains Mono', monospace" }}>ITEM# LAP-00A1</span>
        </div>
        <div style={{ fontSize: 10, color: 'var(--outline)' }}>Category: Electronics</div>
        <div style={{ display: 'flex', gap: 6, marginTop: 'auto' }}>
          ...
          <div style={{ background: 'var(--primary)', borderRadius: 4, padding: '4px 12px', fontSize: 10, fontWeight: 700, color: 'var(--on-primary)', display: 'flex', alignItems: 'center', gap: 4 }}>
            <span className="material-symbols-outlined" style={{ fontSize: 12 }}>shopping_cart</span> Add to Cart
          </div>
        </div>
      </div>
    </div>
...
function PreviewOrders() {
  const rows = [
    { id: 'A2F9-3B1C', item: 'CPU Quantum X9',  status: 'COMPLETED', color: 'var(--secondary)' },
    { id: 'C4D1-9E2A', item: 'NVMe SSD 2TB',    status: 'PROCESSING', color: 'var(--tertiary)' },
    { id: 'F7B3-1C8D', item: 'Ergonomic Chair', status: 'QUEUED',    color: 'var(--primary)' },
    { id: 'E2A5-7F4B', item: 'Lab Centrifuge',  status: 'FAILED',    color: 'var(--error)' },
  ];
  return (
    <div style={{ background: 'var(--surf1)', border: '1px solid var(--border)', borderRadius: 4, overflow: 'hidden' }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {rows.map((r, i) => (
            <tr key={r.id} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 1 ? 'rgba(255,255,255,0.01)' : 'transparent' }}>
              <td style={{ padding: '6px 12px', fontSize: 10, fontFamily: "'JetBrains Mono', monospace", color: 'var(--outline)' }}>{r.id}</td>
              <td style={{ padding: '6px 12px', fontSize: 11, fontWeight: 600, color: 'var(--on-surface)' }}>{r.item}</td>
              <td style={{ padding: '6px 12px' }}>
                <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 7px', borderRadius: 3, background: `color-mix(in srgb, ${r.color} 12%, transparent)`, color: r.color, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{r.status}</span>
              </td>
            </tr>
          ))}
        </tbody>
...
function PreviewEventStream() {
  const events = [
    { icon: 'add_circle',  label: 'ORDER CREATED',    id: 'A2F9', color: 'var(--primary)',   time: '09:41:03' },
    { icon: 'move_to_inbox', label: 'QUEUED',         id: 'A2F9', color: 'var(--outline)',   time: '09:41:03' },
    { icon: 'autorenew',   label: 'PROCESSING',       id: 'A2F9', color: 'var(--tertiary)',  time: '09:41:04' },
    { icon: 'check_circle', label: 'COMPLETED',       id: 'A2F9', color: 'var(--secondary)', time: '09:41:06' },
    { icon: 'add_circle',  label: 'ORDER CREATED',    id: 'B7C3', color: 'var(--primary)',   time: '09:41:07' },
    { icon: 'move_to_inbox', label: 'QUEUED',         id: 'B7C3', color: 'var(--outline)',   time: '09:41:07' },
  ];
  return (
    <div style={{ background: 'var(--surf1)', border: '1px solid var(--border)', borderRadius: 4, overflow: 'hidden' }}>
      <div style={{ padding: '7px 12px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 6 }}>
        <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--secondary)', animation: 'livePulse 2s ease-in-out infinite' }} />
        <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--secondary)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Live</span>
        <span style={{ fontSize: 10, color: 'var(--outline)', marginLeft: 4 }}>6 events</span>
      </div>
      {events.map((e, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 12px', borderBottom: i < events.length - 1 ? '1px solid var(--border)' : 'none' }}>
          ...
          <span style={{ marginLeft: 'auto', fontSize: 10, fontFamily: "'JetBrains Mono', monospace", color: '#464554' }}>{e.time}</span>
        </div>
      ))}
    </div>
  );
...
function PreviewDLQ() {
  const cards = [
    { priority: 'URGENT', badge: 'rgba(255,180,171,0.15)', badgeText: 'var(--error)', border: 'rgba(255,180,171,0.3)', item: 'CPU Quantum X9', msg: 'MAX_RETRIES_EXCEEDED after 5 attempts', retries: 5 },
    { priority: 'ACTION REQUIRED', badge: 'rgba(255,185,95,0.15)', badgeText: 'var(--tertiary)', border: 'rgba(255,185,95,0.2)', item: 'NVMe SSD 2TB', msg: 'INVENTORY_LOCK_TIMEOUT on warehouse write', retries: 3 },
  ];
  return (
    <div style={{ display: 'flex', gap: 10 }}>
      <div style={{ width: 120, flexShrink: 0, background: 'var(--surf1)', border: '1px solid var(--border)', borderRadius: 4, padding: '10px 12px' }}>
        ...
        {[{ label: 'Dead Letter', count: 2, color: 'var(--error)' }, { label: 'High Retries', count: 1, color: 'var(--tertiary)' }, { label: 'Failed', count: 3, color: 'var(--primary)' }].map(s => (
          <div key={s.label} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 7 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: s.color, fontFamily: "'JetBrains Mono', monospace" }}>{s.count}</span>
          </div>
        ))}
      </div>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
        {cards.map(c => (
          <div key={c.item} style={{ background: 'rgba(255,180,171,0.03)', border: `1px solid ${c.border}`, borderLeft: `3px solid ${c.badgeText}`, borderRadius: 4, padding: '10px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              <span style={{ fontSize: 9, fontWeight: 700, background: c.badge, color: c.badgeText, padding: '2px 6px', borderRadius: 3, letterSpacing: '0.05em' }}>{c.priority}</span>
              <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--on-surface)', marginTop: 5, marginBottom: 3 }}>{c.item}</div>
              <div style={{ fontSize: 10, fontFamily: "'JetBrains Mono', monospace", color: 'var(--error)', background: 'rgba(255,180,171,0.08)', padding: '1px 5px', borderRadius: 2 }}>{c.msg}</div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5, flexShrink: 0, marginLeft: 10 }}>
              ...
              <div style={{ fontSize: 9, color: 'var(--outline)', textAlign: 'center' }}>Retries: {c.retries}</div>
            </div>
          </div>
        ))}
      </div>
...
function PreviewAnalytics() {
  const bars = [160, 120, 140, 80, 60];
  const labels = ['Electronics', 'Apparel', 'Consumer', 'Furniture', 'Raw Mat'];
  const maxBar = 160;
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: 10 }}>
      <div style={{ background: 'var(--surf1)', border: '1px solid var(--border)', borderRadius: 4, padding: '12px 14px' }}>
        ...
        <div style={{ fontSize: 26, fontWeight: 800, color: 'var(--primary)', fontFamily: "'JetBrains Mono', monospace", lineHeight: 1, marginBottom: 16 }}>4.8K</div>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: 64 }}>
          {bars.map((h, i) => (
            <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
              <div style={{ width: '100%', height: Math.round((h / maxBar) * 54), background: 'rgba(192,193,255,0.22)', borderRadius: '2px 2px 0 0' }} />
              <span style={{ fontSize: 8, color: 'var(--outline)', textAlign: 'center', lineHeight: 1.2 }}>{labels[i]}</span>
            </div>
          ))}
        </div>
      </div>
      <div style={{ background: 'var(--primary)', border: '1px solid rgba(192,193,255,0.3)', borderRadius: 4, padding: '12px 14px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--on-primary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Order Success Rate</div>
        <svg width="72" height="72">
          ...
          <circle cx="36" cy="36" r="28" fill="none" style={{ stroke: 'var(--on-primary)' }}
            strokeDasharray={`${0.942 * 2 * Math.PI * 28} ${2 * Math.PI * 28}`}
            strokeWidth="6" transform="rotate(-90 36 36)" />
        </svg>
        <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--on-primary)', fontFamily: "'JetBrains Mono', monospace", marginTop: -48, marginBottom: 36 }}>94%</div>
        <div style={{ fontSize: 9, color: 'var(--on-primary)', opacity: 0.7 }}>4,516 completed</div>
        <div style={{ fontSize: 9, color: 'var(--on-primary)', opacity: 0.7 }}>278 failed</div>
      </div>
    </div>
  );
}
...
function PreviewMRP() {
  const materials = [
    { num: 'FG-LAPTOP', desc: 'Laptop Pro Assembly', type: 'PD', oh: '0', color: 'var(--error)' },
    { num: 'CPU-QUANTUM', desc: 'CPU Quantum X9',    type: 'PD', oh: '500', color: 'var(--secondary)' },
    { num: 'RAM-DDR5',   desc: 'DDR5 RAM 32GB',      type: 'PD', oh: '200', color: 'var(--secondary)' },
    { num: 'SSD-NVME',   desc: 'NVMe SSD 2TB',       type: 'PD', oh: '150', color: 'var(--tertiary)' },
  ];
  return (
    <div style={{ background: 'var(--surf1)', border: '1px solid var(--border)', borderRadius: 4, overflow: 'hidden' }}>
      <div style={{ display: 'flex', gap: 4, padding: '7px 10px', borderBottom: '1px solid var(--border)', background: 'var(--border)' }}>
        {['Materials', 'Requirements', 'Planned Orders', 'BOM (9)'].map((t, i) => (
          <span key={t} style={{
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {materials.map((m, i) => (
            <tr key={m.num} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 1 ? 'rgba(255,255,255,0.01)' : 'transparent' }}>
              <td style={{ padding: '5px 10px', fontSize: 9, fontFamily: "'JetBrains Mono', monospace", color: 'var(--primary)', fontWeight: 600 }}>{m.num}</td>
              <td style={{ padding: '5px 10px', fontSize: 10, color: 'var(--on-surface)' }}>{m.desc}</td>
              <td style={{ padding: '5px 10px' }}><span style={{ fontSize: 8, fontWeight: 700, padding: '1px 5px', borderRadius: 2, background: 'rgba(192,193,255,0.1)', color: 'var(--primary)', border: '1px solid rgba(192,193,255,0.2)' }}>{m.type}</span></td>
              <td style={{ padding: '5px 10px', fontSize: 10, fontFamily: "'JetBrains Mono', monospace", color: m.color, fontWeight: 700 }}>{m.oh}</td>
            </tr>
          ))}
        </tbody>
      </table>
    ...
    <div style={{ background: 'var(--surf1)', border: '1px solid var(--border)', borderRadius: 4, overflow: 'hidden' }}>
      ...
      <div style={{ padding: '8px 12px' }}>
        ...
        <div style={{ marginTop: 8, padding: '6px 8px', background: 'rgba(255,185,95,0.08)', border: '1px solid rgba(255,185,95,0.2)', borderRadius: 3 }}>
          <span style={{ fontSize: 9, color: 'var(--tertiary)', fontFamily: "'JetBrains Mono', monospace" }}>[BOM] BOM-EXPLOSION requirements generated for 8 components</span>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ display: 'flex', gap: 10 }}>
      <div style={{ flex: 1, background: 'var(--surf1)', border: '1px solid var(--border)', borderRadius: 4, overflow: 'hidden' }}>
        <div style={{ padding: '8px 12px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between' }}>
          ...
          <span style={{ fontSize: 10, color: 'var(--outline)', fontFamily: "'JetBrains Mono', monospace" }}>3 items</span>
        </div>
        {[
          { name: 'CPU Quantum X9',     price: '$249.00', qty: 2 },
          { name: 'NVMe SSD 2TB',       price: '$189.00', qty: 1 },
          { name: 'Ergonomic Chair Pro', price: '$799.00', qty: 1 },
        ].map(item => (
          <div key={item.name} style={{ padding: '8px 12px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <div style={{ fontSize: 10, color: 'var(--outline)', fontFamily: "'JetBrains Mono', monospace" }}>{item.price} each</div>
            </div>
...
const STEPS: Step[] = [
  ...
  },
  {
    ...
    nav: 'Catalog',
    navIcon: 'storefront',
    tagline: 'Select a SKU, review inventory, and submit directly to the live queue.',
    description: 'The Catalog provides a master-detail browser over the product inventory. The left panel lists all available SKUs with current stock status and warehouse count. Selecting a product loads its full detail view: inventory levels, warehouse distribution, stock movement history, and technical specifications.',
    instructions: [
      ...
      'Review the stock status badge (IN STOCK / LOW STOCK / CRITICAL), on-hand quantity, and warehouse distribution table.',
